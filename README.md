...
In the file controlled_R_y_target_N.cpp there are the functions to decompose an arbitrary quantum system |Q> in a set of block spheres
...
all the solutions of a 3-Sat instance is quite smaller than 2^N. Nevertheless it increases exponentially with complexity $O(a^N)$ with $a \approx 1.65$ for all $p<1.0$. Even when $p=1.0$ the number of bloch spheres increases exponetially with $N$ with complexity $O(b^N)$ where $b \approx 1.23$
